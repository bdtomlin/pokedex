...
func TestCmdMapPage2(t *testing.T) {
	cfg := newTestConfig(pokeapi.NewPokeApi(pokecache.NewTestCache()))
...
func TestCmdMapInvalid(t *testing.T) {
	cfg := newTestConfig(pokeapi.NewPokeApi(pokecache.NewTestCache()))
	cfg.Next = "lwkejffkejfwlkjef"
	err := cmdMap(cfg)
	if err == nil {
...
func TestCmdMapNoNext(t *testing.T) {
	cfg := newTestConfig(pokeapi.NewPokeApi(pokecache.NewTestCache()))
...
Next
`
	cfg.Next = "https://pokeapi.co/api/v2/location-area?offset=1050&limit=20"
	err := cmdMap(cfg)
	if err != nil {
